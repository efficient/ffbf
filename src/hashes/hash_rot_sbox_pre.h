...
#ifndef _HASH_ROT_SBOX_PRE_H_
#define _HASH_ROT_SBOX_PRE_H_

#include "hash_common.h"
#include "hash_buf.h"

template<int LEN> class hash_rot_sbox_pre {
private:
...
public:
    ...
    /* Optimized version of the earlier hash_rot_sbox.  This version
     * computes the "rotate out" value while it has the sbox[c] value
     * already in register, instead of deferring it to the next function
     * call.  Roughly reduces the length of the function from two
     * sequential L1 lookups to one L1 lookup.  Uses more space (19 words
     * instead of 19 bytes) than the old version, but seems to be a
     * useful speedup in practice.  On my 2.53Ghz Macbook, in a best-case
     * test (no reset), goes from 143MB/sec to 245MB/sec. -dga
     */
    inline void update(unsigned char c) {
        h = rol32(h, 1);
        h ^= sbox[c];
        h ^= b[bufptr];
        b[bufptr++] = rol32(sbox[c], LEN);
        if (bufptr >= LEN) {
            bufptr = 0;
        }
    }
...
template <int LEN> const uint32_t hash_rot_sbox_pre<LEN>::sbox[] = {
