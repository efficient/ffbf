#ifndef _HASH_ROT_SBOX_H_2_
#define _HASH_ROT_SBOX_H_2_

#include "hash_common.h"
#include "hash_buf.h"
#include <stdint.h>

template<int LEN> class hash_rot_sbox_2 {
private:
...
public:
    ...
    hash_rot_sbox_2() { rotate_old = 2 * LEN % 32; }

    inline uint32_t hval1() { return h1; }

    inline uint32_t hval2() { return h2; }
    ...
    inline void update(unsigned char c) {
        ...
        if (b.is_full()) {
            ...
            uint32_t tmp = sbox[old];
            tmp = rol32(tmp, LEN);
            h1 ^= tmp;
            tmp = rol32(tmp, rotate_old);
            h2 ^= tmp;
        } else {
            b.push(c);
        }
    ...
    inline bool is_full() { return b.is_full(); }
};

template <int LEN> const uint32_t hash_rot_sbox_2<LEN>::sbox[] = {
